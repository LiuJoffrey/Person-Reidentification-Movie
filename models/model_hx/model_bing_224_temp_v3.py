...
class QKV(nn.Module):
    def __init__(self,fs=5,max_c = 11):
        ...
        self.down = kqkv2class(self.max_c,self.fs)
...
#            tqkv_.append(qkv.view(bs,-1).unsqueeze(0))
        ...
        c_pred_ = c_pred_.view(bs*fs,self.max_c+1+fs,16,16)
...
class kqkv2class(nn.Module):
    def __init__(self,max_c,fs):
        super(kqkv2class, self).__init__()
        self.downs = nn.Sequential(nn.Conv2d(12+fs,256,1,padding=0),
                                   nn.BatchNorm2d(256),
                                   nn.LeakyReLU(0.2),
                                   nn.Dropout(0.5),
