...
def extract_feature(imgs, model, device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu"), tta = True):
    # pre-requisites
    ...
    # with torch.no_grad():
    #     if tta:
    #         emb_batch = model(imgs.to(device)).cpu() + model(flipped.to(device)).cpu()
