...
model_body = r_net50(pretrained=False)
# model_body = r_net101(pretrained=False)
...
for i, batch in trange:
    ...
    with torch.no_grad():
        ...
        # with open("feature_save/test6/cast/face/"+movie_name+".pickle", 'wb') as f:
        #     pickle.dump(cast_det_img, f)
        
        #### Get cast body feature ###
        cast_body_feature = extract_body_feature(cast_img.cuda(), model_body)
        ...
        # with open("feature_save/test6/candidate/face/"+movie_name+".pickle", 'wb') as f:
        ...
        for cast_idx in range(len(face_sort)):
            ...
            for candidate_idx in face_sort[cast_idx]:
                # print(cast_idx, face_result[cast_idx, candidate_idx])
                # break
                # cv2.imshow("x", movie_candidate_detect_img[len(movie_candidate_detect_img)-candidate_idx-1])
                # cv2.waitKey(0)
                # continue
                # # continue
                # exit()
                if face_result[cast_idx, candidate_idx] > 2.06:
                    ...
                    # print(cast_idx, face_result[cast_idx, candidate_idx])
                    # cv2.imshow("x", movie_candidate_detect_img[candidate_idx])
                    # cv2.waitKey(0)
        ...
        for cast_idx, cand_len in enumerate(new_cand_to_cast_len):
            if cand_len!=0:
                ...
                # candidate_score = torch.mean(same_cast, 0).unsqueeze(0)
                # cast_with_candidate_score = ct_similarity[cast_idx].unsqueeze(0) *0.8 + candidate_score*0.2

                # cast_donimate_similarity_max.append(cast_with_candidate_score)
                cast_donimate_similarity_max.append(torch.max(same_cast, 0)[0].unsqueeze(0))
...
import json    
with open("output/face_ppcc/val_split.json", 'w') as f:
    json.dump(result_id, f)
      
import csv
submission = open(os.path.join(out_file, "submit_val_face_propagation_split.csv"), "w+") # "./{}.csv".format(target_dataset_name)
